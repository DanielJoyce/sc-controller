...
download_dep "python-gobject-3.26.1" "https://archive.archlinux.org/packages/p/python3-gobject/python3-gobject-3.26.1-1-x86_64.pkg.tar.xz"
...
# Prepare & build deps
export PYTHONPATH=${BUILD_APPDIR}/usr/lib/python3.7/site-packages/
mkdir -p "$PYTHONPATH"
if [[ $(grep ID_LIKE /etc/os-release) == *"suse"* ]] ; then
	# Special handling for OBS
	ln -s lib64 ${BUILD_APPDIR}/usr/lib
	export PYTHONPATH="$PYTHONPATH":${BUILD_APPDIR}/usr/lib64/python3.7/site-packages/
	LIB=lib64
fi
...
cp "/usr/include/linux/input-event-codes.h" ${BUILD_APPDIR}/usr/${LIB}/python3.7/site-packages/scc/
